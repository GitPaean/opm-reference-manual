...
3)  1)  Restrict the change allowed per iteration and prevent the solution from jumping far across the boundary between saturated and undersaturated states in a single iteration. Thus, like the commercial simulator, OPM Flow implements the Appleyard[^22] chop technique, which is often sufficient for the method to converge. However, time step chops[^23] may still occur in order to obtain convergence for some difficult cases, particularly for models with very fine grids.
    ...
    3)  Thirdly, and frequently the main cause of non-convergence and the lack of smoothness of the residuals, is the discontinuity or inconsistent input data, particularly the relative permeability and capillary pressure functions. Thus, using Corey[^24] or Lomeland et al.[^25] type of curves for these data may well improve convergence performance, see SGOFLET, SGWFLET, and SWOFLET, for OPM Flow's implementation of the Lomeland type of curves.
...
For more information about run time options and numerical controls, see section [Running OPM Flow From The Command Line](#running-opm-flow-from-the-command-line) on how to invoke various numerical schemes via the OPM Flow command line interface.
...
By default, BiCG-stab is used. In addition, there are several options available for preconditioning the linear system, such as the incomplete lower triangular-upper triangular factorization with level of fill in 0 (ILU0). The default option is based on an algebraic multi-grid (AMG) method for preconditioning, named the Constraint Pressure Residual (CPR) preconditioner (originally introduced by Wallis[^26] using the approach of Scheichl et al.[^27]). The solvers and preconditioners are implemented in the Dune module dune-istl and described by Blatt et al.[^28], [^29], and [^30]. The interface between OPM Flow and the linear solver packages has been designed in a flexible way so that besides dune-istl other linear algebra packages could be used.

For more information about run time options and numerical controls, see section [Running OPM Flow From The Command Line](#running-opm-flow-from-the-command-line) on how to invoke various numerical schemes via the OPM Flow command line interface. In addition, section [Improving Simulator Convergence and Numerical Performance](#improving-simulator-convergence-and-numerical-performance) provides some practical advice on how to address numerical performance issues.
...
[^10]: Connections between cells due to geometric adjacency are automatically calculated by the simulator, including across faults in the grid. In addition, the user can specify extra connections between arbitrary cells using the NNC keyword in the GRID section.
