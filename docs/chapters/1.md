...
3)  1)  Restrict the change allowed per iteration and prevent the solution from jumping far across the boundary between saturated and undersaturated states in a single iteration. Thus, like the commercial simulator, OPM Flow implements the Appleyard[^22] chop technique, which is often sufficient for the method to converge. However, time step chops[^23] may still occur in order to obtain convergence for some difficult cases, particularly for models with very fine grids.
    ...
    3)  Thirdly, and frequently the main cause of non-convergence and the lack of smoothness of the residuals, is the discontinuity or inconsistent input data, particularly the relative permeability and capillary pressure functions. Thus, using Corey[^24] or Lomeland et al.[^25] type of curves for these data may well improve convergence performance, see [SGOFLET](#__RefHeading___Toc398952_3017686537), [SGWFLET](#__RefHeading___Toc548127_947687768), and [SWOFLET](#__RefHeading___Toc398954_3017686537), for OPM Flow's implementation of the Lomeland type of curves.
