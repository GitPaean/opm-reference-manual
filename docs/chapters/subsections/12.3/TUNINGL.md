...
[TUNINGL](#__RefHeading___Toc1647717_4250154414) defines the parameters used for controlling the commercial simulator's numerical convergence parameters for all Local Grid Refinements (\"LGR\"). The keyword is the same as the [TUNING](#__RefHeading___Toc146744_4203985108) keyword in the [SCHEDULE](#__RefHeading___Toc43945_784232322) section that applies the tuning parameters to the global grid. See section [Running OPM Flow From The Command Line](#running-opm-flow-from-the-command-line) on how to invoke various numerical schemes via the OPM Flow command line interface.

This keyword is not supported by OPM Flow but has no effect on the results so it will be ignored.

| 1-1 | TSINIT | TSINT is a real positive value that defines the maximum length of the next time step.<br>Note that whenever the keyword is used TSINIT is always set back to the default value of one, unless explicitly over written. | 1.0 |
| --- | --- | --- | --- |
| days | days | hours |  |
...
| dimensionless | dimensionless | dimensionless |  |
| 1-9 | THRURPT | THRURPT is a real positive value that specifies the maximum throughput ratio over a time step. | 1.0 x 10^20^ |
| dimensionless | dimensionless | dimensionless |  |
...
| dimensionless | dimensionless | dimensionless |  |
| 2-13 | TRWGHT | TRWGHT is a positive integer that stipulates the implicitness for active tracer updates within the Newton iterations, and should be set to:<br>0.  The calculation is explicit, that is fully decoupled.<br>1.  The calculation is implicit, that is fully coupled. | 1 |
| dimensionless | dimensionless | dimensionless |  |
