...
Here the GASFDECR keyword allows for a reduction in the calculated final pass monthly gas rates and thus equation become:
...
SWINGFAC~month\ ~= monthly rate scaling factor that takes into account seasonal demand,

etc.

GASFDECR~month~ = monthly gas rate reduction.
