...
| 13 | FIPNUM | An integer value defines the FIPNUM region used to determine the reservoir conditions in calculating the well's reservoir volumes and is determined by:<br>1)  If set to a negative integer value then the FIPNUM region of the deepest connection in the well will be used.<br>2)  If set to zero, the default value, then the average properties for the field will be used.<br>3)  If set to an integer value greater than zero, then the FIPNUM indicated by this value will be used. | 0 |
