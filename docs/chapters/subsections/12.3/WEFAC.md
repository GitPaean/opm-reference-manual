...
| 3 | WELNETWK | A defined character string that determines if the WELNAME efficiency factor should be applied when calculating the flows and pressure losses in the Extended Network Model, and should be set to either:<br>1)  NO: The well's equivalent Extended Network Model node flow rates are not multiplied by the efficiency factor FACTOR.<br>2)  YES: The well's equivalent Extended Network Model node flow rates are multiplied by the efficiency factor FACTOR<br>This option is only applicable for the Extended Network Model, as in the Standard Network Model groups flow rates are always used in the calculation of pressure drops (this is equivalent to the default option, YES). | YES |
