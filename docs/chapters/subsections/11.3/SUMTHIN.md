...
| 1 | SUMSTEP | SUMSTEP is a real positive number that defines the time interval for which the first time step of data will be written to the [SUMMARY](#__RefHeading___Toc43949_784232322) file (and the RSM file if RSM output has been activated).<br>For example, if SUMSTEP is set to 30 days, and if the simulator takes time steps of 0, 5, 10, 16, 24, 30, 40, 45, 60, 90 days. Then the [SUMMARY](#__RefHeading___Toc43949_784232322) data will be written out at time steps 0, 30, 40 and 60 days. | None |
| days | days | hours |  |
