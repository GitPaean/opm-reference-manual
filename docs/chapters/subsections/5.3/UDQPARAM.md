...
| 1 | RSEED | RSEED is a positive integer greater than zero that sets a new random number seed for use in the UDQ functions RANDN, RANDU RRNDN and RRNDU.<br>See also the RSEED character variable on the UDQDIMS keyword in the RUNSPEC section to default the random number seed for a restart run.<br>This feature is not supported by OPM Flow. | 1 |
| --- | --- | --- | --- |
