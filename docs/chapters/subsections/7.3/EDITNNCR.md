...
| 14 | DIFFNNC | DIFFNNC is a positive real number greater than or equal to zero that scales the diffusivity between the first grid block (I1, J1, K1) and the second grid block (I2, J2, K2).<br>The default value is the value calculated in the GRID section. | 1\* |
| feet | meters | cm |  |
