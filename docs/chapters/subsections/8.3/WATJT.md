...
WATJT activates the water Joule-Thomson effect[^1] in temperature calculations, and defines the water Joule-Thomson Coefficient ("JTC") at a given reference pressure, for when OPM Flow's thermal option has been activated by the THERMAL keyword in the RUNSPEC.
...
| psia | barsa | atma |  |
| 2 | WATJTC | WATJTC is a real positive or negative value that defines the water phase Joule-Thomson Coefficient.<br>If the value is defaulted (1\*) or set to 0, then WATJTC is internally calculated using thermal water density data on the WATSDENT keyword in the PROPS section.<br>If a non-zero value is specified, then the WATJTC is assumed to be constant and equal to that value. | 0 |
