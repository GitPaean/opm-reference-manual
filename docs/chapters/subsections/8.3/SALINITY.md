...
| 1 | [SALINITY](#__RefHeading___Toc405185_1616145207) | A real positive value that defines the salinity for all grid blocks in the model for when the CO~2~-Brine model has been activated.<br>Note that the units for salinity are molality, that is gm-M/Kg, and therefore the units are defined as given below with the 10^-3^ prefix. |  |  |  |
