...
| psia | barsa | atma |  |
| Notes:<br>1)  The keyword is followed by NTPVT rows as declared on the TABDIMS keyword in the RUNSPEC section.<br>2)  Each row must contain two values representing the RV and PRESS variables.<br>3)  Each row is terminated by a "/" and there is no "/" terminator for the keyword. |  |  |  |
