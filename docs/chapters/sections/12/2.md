...
source: parts/chapters/sections/12/2.fodt
generated_by: scripts/python/src/fodt/fodt2md
---

## <a id="-refheading-toc119430-2179381650"></a>Data Requirements

Apart from the keywords that advance the simulator through time \(the [DATES](#-refheading-toc117621-2179381650), [TIME](#-refheading-toc1252966-4250154414), and [TSTEP](#-refheading-toc118323-1596574740) keywords\),  the minimum required data is associated with defining a well \([WELSPECS](#-refheading-toc268463-1366622701)\), defining a well’s connection to the reservoir \([COMPDAT](#-refheading-toc97651-3261743917)\), and the operating and production targets and constraints for the well \([WCONHIST](#-refheading-toc134880-2055188184),  [WCONPROD](#-refheading-toc146754-4203985108), [WCONINJH](#-refheading-toc146752-4203985108), or [WCONINJE](#-refheading-toc146750-4203985108)\).  Well connections can be “grouped” into completions using the [COMPLUMP](#-refheading-toc97655-3261743917) keyword to represent actual physical well completions in the model. Wells can either operate independently or under group control. That is when a well is allocated to a group, then the group can dictate how the wells in the group under group control are operated. For example, a group may have production targets and constraints and all wells under group control within the group will be operated in such a manner as to satisfy the group’s targets and constraints. Note that wells can belong to a group but do not necessary have to be under group control. The top level group, level one, has the name FIELD and under this level can be wells, groups, and sub groups to the higher level groups. By default three group levels are defined that sets the wells as level three, reporting directly to defined groups at level two, and the level two groups reporting to the FIELD group at level one.  If a different configuration is required, then the [GRUPTREE](#-refheading-toc118321-1596574740) keyword should be used to define the group hierarchy by defining a lower level group that reports directly to a higher level group.  
...
| --- | --- | --- |
| Vertical Flow Tubing Performance Table Specification | [VFPINJ](#-refheading-toc121917-2556401936) | [VFPINJ – Define Injection Vertical Flow Performance Tables](#11351vfpinj-define-injection-vertical-flow-performance-tables). [VFPINJ](#-refheading-toc121917-2556401936) declares similar data to [VFPPROD](#-refheading-toc121919-2556401936) but for injection wells. |
| [VFPPROD](#-refheading-toc121919-2556401936) | [VFPPROD – Define Production Vertical Flow Performance Tables](#11352vfpprod-define-production-vertical-flow-performance-tables). The [VFPPROD](#-refheading-toc121919-2556401936) keyword defines production Vertical Flow Performance \(“VFP”\) tables that are used to determine the outflow or downstream pressure based on the inlet or upstream pressure and the phases flowing through the system.  For a well this means the table relates the flowing bottom\-hole pressure \(“FBHP”\) to the well’s tubing head pressure \(“THP”\) based on the oil, gas, and water rates \(and any artificial lift quantities like gas lift gas\), or phases ratios, flowing up the wellbore. |  |
| Well Specification ‍ | [WELSPECS](#-refheading-toc268463-1366622701) | [WELSPECS – Define Well Specifications](#11365welspecs-define-well-specifications). The [WELSPECS](#-refheading-toc268463-1366622701) keyword defines the general well specification data for all well types, and must be used for all wells before any other well specification keywords are used in the input file. The keyword declares the name of the well, the initial group the well belongs to, the wellhead location and other key parameters. |
| Well Completion Data ‍ ‍ | [COMPDAT](#-refheading-toc97651-3261743917) | [COMPDAT – Define Well Connections to the Grid](#1133compdat-define-well-connections-to-the-grid). [COMPDAT](#-refheading-toc97651-3261743917) defines how a well is connected to the reservoir by defining or modifying existing well connections. |
| [COMPLUMP](#-refheading-toc97655-3261743917) | [COMPLUMP – Assign Well Connections to Completions](#1135complump-assign-well-connections-to-completions). The [COMPLUMP](#-refheading-toc97655-3261743917) keyword assigns connections, as defined by the [COMPDAT](#-refheading-toc97651-3261743917) keyword, to completion intervals. This “lumping” or “grouping” of the connections to various completion intervals allows automatic workovers and economic criteria to be applied to the completions \(that is a set of connections\) as opposed to the individual connections. |  |
| [COMPTRAJ](#-refheading-toc97651-32617439171) | [COMPTRAJ – Define Well Trajectory Connections to the Grid](#12338comptraj-define-well-trajectory-connections-to-the-grid). [COMPTRAJ](#-refheading-toc97651-32617439171) keyword defines how a well that has been declared as a trajectory well, using the [WELTRAJ](#-refheading-toc268463-13666227011) keyword, is connected to the reservoir model by defining or modifying existing well perforation depths. The keyword can only be used for wells defined by the [WELTRAJ](#-refheading-toc268463-13666227011) keyword, This is an OPM Flow specific keyword and will therefore cause an error in the commercial simulator. |  |
| [WELTRAJ](#-refheading-toc268463-13666227011) | [WELTRAJ – Define Well Trajectory Data](#123285weltraj-define-well-trajectory-data). [WELTRAJ](#-refheading-toc268463-13666227011) defines a trajectory well together with the well trajectory data, and is used in conjunction with the [COMPTRAJ](#-refheading-toc97651-32617439171) keyword to define the well connections to the simulation grid blocks. The keyword can only be used for trajectory wells that employ the [COMPTRAJ](#-refheading-toc97651-32617439171) keyword to define the connections to the grid, This is an OPM Flow specific keyword and will therefore cause an error in the commercial simulator. |  |
| Historical Production and Injection Data | [WCONHIST](#-refheading-toc134880-2055188184) | [WCONHIST – Define Well Historical Production Rates and Pressures](#11353wconhist-define-well-historical-production-rates-and-pressures). [WCONHIST](#-refheading-toc134880-2055188184) defines production rates and pressures for wells that have been declared history matching wells by the use of this keyword. History matching wells are handled differently than ordinary wells that use the [WCONPROD](#-refheading-toc146754-4203985108) keyword for controlling their production targets and constraints. |
| [WCONINJH](#-refheading-toc146752-4203985108) | [WCONINJH – Well Historical Observed Injection Rates and Pressures](#11356wconinjh-well-historical-observed-injection-rates-and-pressures). [WCONINJH](#-refheading-toc146752-4203985108) declares similar data as the [WCONHIST](#-refheading-toc134880-2055188184) keyword but for injection wells. |  |
| Predictive Production and Injection Data | [WCONPROD](#-refheading-toc146754-4203985108) | [WCONPROD – Define Well Production Targets and Constraints](#11357wconprod-define-well-production-targets-and-constraints). The [WCONPROD](#-refheading-toc146754-4203985108) keyword defines production targets and constraints for wells that have previously been defined by the [WELSPECS](#-refheading-toc268463-1366622701) keyword. |
| [WCONINJE](#-refheading-toc146750-4203985108) | [WCONINJE – Well Injection Targets and Constraints](#11355wconinje-well-injection-targets-and-constraints). [WCONINJE](#-refheading-toc146750-4203985108) declares similar data as the [WCONPROD](#-refheading-toc146754-4203985108) keyword but for injection wells. |  |
| Well Control | [WELCNTL](#-refheading-toc134886-2055188184) | [WELCNTL – Modify Well Control and Targets](#11362welcntl-modify-well-control-and-targets). The [WELCNTL](#-refheading-toc134886-2055188184) keyword modifies a well’s target control and value, both rates and pressures, for previously defined wells without having to define all the variables on the well control keywords: [WCONPROD](#-refheading-toc146754-4203985108), [WCONHIST](#-refheading-toc134880-2055188184), [WCONINJE](#-refheading-toc146750-4203985108), or [WCONINJH](#-refheading-toc146752-4203985108) keywords. See also the [WELTARG](#-refheading-toc134888-2055188184) keyword below. |
| [WECON](#-refheading-toc134884-2055188184) | [WECON – Well Economic Criteria for Production Wells](#11360wecon-well-economic-criteria-for-production-wells). [WECON](#-refheading-toc134884-2055188184) defines the economic criteria for production wells that have previously been defined by the [WELSPECS](#-refheading-toc268463-1366622701) and [WCONPROD](#-refheading-toc146754-4203985108) keywords. |  |
| [WEFAC](#-refheading-toc48856-327352552) | [WEFAC – Define Well Efficiency](#10211wefac-define-well-efficiency). [WEFAC](#-refheading-toc48856-327352552) defines a well’s efficiency or up\-time factor as opposed to setting the efficiency factors at the group level. |  |
| [WHISTCTL](#-refheading-toc268465-1366622701) | [WHISTCTL \- Define Well Historical Target Phase](#11369whistctl---define-well-historical-target-phase). The [WHISTCTL](#-refheading-toc268465-1366622701) keyword changes the target control phase for wells declared as history match wells via the [WCONHIST](#-refheading-toc134880-2055188184) keyword. The target phase is set on the [WCONHIST](#-refheading-toc134880-2055188184) keyword and [WHISTCTL](#-refheading-toc268465-1366622701) overrides this value for all subsequent entries on the [WCONHIST](#-refheading-toc134880-2055188184) keyword. |  |
| [WELOPEN](#-refheading-toc268461-1366622701) | [WELOPEN – Define Well and Well Connections Flowing Status](#11363welopen-define-well-and-well-connections-flowing-status). [WELOPEN](#-refheading-toc268461-1366622701) defines the status of wells and the well connections, and is used to open and shut previously defined wells and well connections without having to re\-specify all the data on the well control keywords: [WCONPROD](#-refheading-toc146754-4203985108),  [WCONHIST](#-refheading-toc134880-2055188184), [WCONINJE](#-refheading-toc146750-4203985108), or [WCONINJH](#-refheading-toc146752-4203985108) keywords. |  |
| [WELTARG](#-refheading-toc134888-2055188184) | [WELTARG – Modify Well Target and Constraint Values](#11366weltarg-modify-well-targets-and-constraints-values). The [WELTARG](#-refheading-toc134888-2055188184) keyword modifies the target and constraints values of both rates and pressures for previously defined wells without having to define all the variables on the well control keywords: [WCONPROD](#-refheading-toc146754-4203985108),  [WCONHIST](#-refheading-toc134880-2055188184), [WCONINJE](#-refheading-toc146750-4203985108), or [WCONINJH](#-refheading-toc146752-4203985108) keywords. See also the [WELCNTL](#-refheading-toc134886-2055188184) keyword above. |  |
| Well Productivity Adjustments \(Normally used in History Matching\) | [WELPI](#-refheading-toc121389-332691817) | [WELPI – Define Well Productivity and Injectivity Indices](#12371welpi-define-well-productivity-and-injectivity-indices). [WELPI](#-refheading-toc121389-332691817) is used to define a well’s productivity or injectivity index and the values entered on this keyword for a given well will override any previously calculated values, or values previously entered using this keyword. |
| [WPIMULT](#-refheading-toc121645-2412586160) | [WPIMULT – Define Well Connection Multipliers](#11374wpimult-define-well-connection-multipliers). The [WPIMULT](#-refheading-toc121645-2412586160) keyword defines a well connection multiplier factor that scales the existing well connection values. The resulting effect is to scale the well’s productivity at the reporting time step the keyword is entered. |  |
| Notes: - Cells colored orange under the Keywords column are currently not supported by OPM Flow. |  |  |

Table 12.1: Major Well Specification Keywords

Wells are initially allocated to groups via the [WELSPECS](#-refheading-toc268463-1366622701) keyword and groups have a similar set of keywords as for wells, as outlined in Table 12.2. However only a limited set of keywords have been implemented in OPM Flow compared with the commercial simulator’s set of keywords.
| Major Group Specification Keywords |  |  |
| --- | --- | --- |
| Group Specification | [GRUPTREE](#-refheading-toc118321-1596574740) | [GRUPTREE – Define Group Tree Hierarchy](#11318gruptree-define-group-tree-hierarchy). [GRUPTREE](#-refheading-toc118321-1596574740) defines the hierarchy of groups that have been created by having wells assigned to them via the [WELSPECS](#-refheading-toc268463-1366622701) keyword. By default three group levels are defined that sets the wells as level three, reporting directly to defined groups at level two, and the level two groups reporting to the FIELD group at level one. |
| Predictive Production and Injection Data | [GCONPROD](#-refheading-toc146746-4203985108) | [GCONPROD – Group Production Targets and Constraints](#11313gconprod-group-production-targets-and-constraints). The [GCONPROD](#-refheading-toc146746-4203985108) keyword defines production targets and constraints for groups, including the top most group in the group hierarchy known as the FIELD group. |
| [GCONINJE](#-refheading-toc134874-2055188184) | [GCONINJE – Group Injection Targets and Constraints](#11312gconinje-group-injection-targets-and-constraints). [GCONINJE](#-refheading-toc134874-2055188184) declares similar data as for [GCONPROD](#-refheading-toc146746-4203985108) but for group injection targets and constraints. |  |
| Group Control | [GECON](#-refheading-toc134876-2055188184) | [GECON – Group Economic Criteria for Production Groups](#11314gecon-group-economic-criteria-for-production-groups). The [GECON](#-refheading-toc134876-2055188184) keyword defines economic criteria for production groups, including the field level group FIELD, that have previously been defined by the [WELSPECS](#-refheading-toc268463-1366622701) and [GCONPROD](#-refheading-toc146746-4203985108) keyword. |
| [GEFAC](#-refheading-toc268455-1366622701) | [GEFAC – Define Group Efficiency](#11315gefac-define-group-efficiency). [GEFAC](#-refheading-toc268455-1366622701) defines a group’s efficiency or up\-time factor as opposed to setting the efficiency factors for individual wells. |  |
| [GRUPTARG](#-refheading-toc196552-870710203) | [GRUPTARG – Modify Group Targets and Constraints Values](#123103gruptarg-modify-group-targets-and-constraints-values). [GRUPTARG](#-refheading-toc196552-870710203) keyword modifies the production targets and constraints of both rates and pressures for previously defined groups without having to define all the variables on the group production control keywords: [GCONPROD](#-refheading-toc146746-4203985108) or [GCONPRI](#-refheading-toc659214-1190369742) keywords. |  |
| Notes: - Cells colored orange under the Keywords column are currently not supported by OPM Flow. |  |  |
...
| --- | --- | --- |
| Control [RS](#-refheading-toc137361-1317547213) and [RV](#-refheading-toc137365-1317547213) Behavior | [DRSDT](#-refheading-toc117623-2179381650) | [DRSDT – Solution Gas \(Rs\) Maximum Rate of Increase Parameters](#1139drsdt-solution-gas-rs-maximum-rate-of-increase-parameters). [DRSDT](#-refheading-toc117623-2179381650) defines the maximum rate at which the solution gas\-oil ratio \(Rs\) can be increased in a grid cell. The keyword is similar in functionality to the [DRSDTR](#-refheading-toc135414-4268459800) keyword, that defines the maximum rate at which Rs can be increased in a grid cell by region. |
| [DRSDTR](#-refheading-toc135414-4268459800) | [DRSDTR – Solution Gas \(Rs\) Maximum Rate of Increase Parameters by Region](#12317drsdtr-solution-gas-rs-maximum-rate-of-increase-parameters-by-region). [DRSDTR](#-refheading-toc135414-4268459800) defines the maximum rate at which the solution gas\-oil ratio \(Rs\) can be increased in a grid cell for various regions in the model.  The keyword is similar in functionality to the [DRSDT](#-refheading-toc117623-2179381650) keyword, that defines the maximum rate at which Rs can be increased in a grid cell for all cells in the model. |  |
| [DRVDT](#-refheading-toc117625-2179381650) | [DRVDT – Solution Oil \(Rv\) Maximum Rate of Increase Parameters](#11310drvdt-solution-oil-rv-maximum-rate-of-increase-parameters). [DRVDT](#-refheading-toc117625-2179381650) defines the maximum rate at which the solution oil\-gas ratio or condensate\-gas ratio \(Rv\) can be increased in a grid cell. The keyword is similar in functionality to the [DRVDTR](#-refheading-toc135416-4268459800) keyword, that defines the maximum rate at which Rv can be increased in a grid cell by region. |  |
| [DRVDTR](#-refheading-toc135416-4268459800) | [DRVDTR – Solution Oil \(Rv\) Maximum Rate of Increase Parameters by Region](#12319drvdtr-solution-oil-rv-maximum-rate-of-increase-parameters-by-region). [DRVDTR](#-refheading-toc135416-4268459800) defines the maximum rate at which the solution oil\-gas ratio or condensate\-gas ratio \(Rv\) can be increased in a grid cell for various regions in the model.  The keyword is similar in functionality to the [DRVDT](#-refheading-toc117625-2179381650) keyword, that defines the maximum rate at which Rv can be increased in a grid cell for all cells in the model. |  |
| Schedule Advancement | [DATES](#-refheading-toc117621-2179381650) | [DATES – Advance Simulation by Reporting Date](#1138dates-advance-simulation-by-reporting-date). [DATES](#-refheading-toc117621-2179381650) advances the simulation to a given report date after which additional keywords may be entered to instruct OPM Flow to perform additional functions via the [SCHEDULE](#-refheading-toc43945-784232322) section keywords, or further [DATES](#-refheading-toc117621-2179381650) keywords may be entered to advance the simulator to the next report date. |
| [TIME](#-refheading-toc1252966-4250154414) | [TIME – Advance Simulation by Cumulative Reporting Time](#123203time-advance-simulation-by-cumulative-reporting-time). [TIME](#-refheading-toc1252966-4250154414) advances the simulation to a given cumulative report time after which additional keywords may be entered to instruct OPM Flow to perform additional functions via the [SCHEDULE](#-refheading-toc43945-784232322) section keywords, or further [TIME](#-refheading-toc1252966-4250154414) keywords may be entered to advance the simulator to the next report time. |  |
| [TSTEP](#-refheading-toc118323-1596574740) | [TSTEP – Advance Simulation by Reporting Time](#11347tstep-advance-simulation-by-reporting-time). [TSTEP](#-refheading-toc118323-1596574740) keyword advances the simulation to a given report time after which additional keywords may be entered to instruct OPM Flow to perform additional functions via the [SCHEDULE](#-refheading-toc43945-784232322) section keywords, or further [TSTEP](#-refheading-toc118323-1596574740) keywords may be entered to advance the simulator to the next report time |  |
| Schedule Advancement Control | [TUNING](#-refheading-toc146744-4203985108) | [TUNING \- Numerical Tuning Control](#11348tuning---numerical-tuning-control). [TUNING](#-refheading-toc146744-4203985108) defines the parameters used for controlling the commercial simulator’s numerical convergence parameters for the global grid. The keyword is mostly ignored by OPM Flow; however, the simulator can be instructed to read the [TUNING](#-refheading-toc146744-4203985108) keyword if the appropriate command line parameter has been activated \(see section [2.2 Running OPM Flow 2023\-04 From The Command Line](#22running-opm-flow-2018-10)\). |
| [NEXTSTEP](#-refheading-toc323446-1841740821) | [NEXTSTEP – Maximum Next Time Step Size](#123147nextstep-maximum-next-time-step-size). [NEXTSTEP](#-refheading-toc323446-1841740821) defines the maximum time step size the simulator should take for the next time step. This keyword can be used to reset the time step for when known large changes to the model are taking place that may result in time step chops. |  |
| Reporting | [RPTSCHED](#-refheading-toc268459-1366622701) | [RPTSCHED – Define SCHEDULE Section Reporting](#11341rptsched-define-schedule-section-reporting). [RPTSCHED](#-refheading-toc268459-1366622701) keyword defines the data in the [SCHEDULE](#-refheading-toc43945-784232322) section that is to be printed to the output print file in human readable format. |
| [RPTRST](#-refheading-toc210154-2884651453) | [RPTRST – Define Data to be Written to the RESTART File](#9211rptrst-define-data-to-be-written-to-the-restart-file). [RPTRST](#-refheading-toc210154-2884651453) keyword defines the data and frequency of the data to be written to the [RESTART](#-refheading-toc135629-1317547213) file at each requested restart point. In addition to the solution data arrays required to restart a run, the user may request additional data to be written to the restart file for visualization in OPM ResInsight. |  |
| Notes: - Cells colored orange under the Keywords column are currently not supported by OPM Flow. |  |  |

Table 12.3: Schedule Advancement, Control, and Reporting Keywords

In terms of structuring the format of the keywords in the [SCHEDULE](#-refheading-toc43945-784232322) section it is advisable to declared all the VLP tables, wells and groups at the start of [SCHEDULE](#-refheading-toc43945-784232322) section as oppose to declaring the items as they are needed, or when they come on stream at various times during the simulation. This produces a cleaner input deck and tends to limit unforeseen errors, as all items are declared upfront and only the operational changes \(opening wells, changing group and well targets, etc.\) are needed. The example [SCHEDULE](#-refheading-toc43945-784232322) section given on the following pages illustrates a typical [SCHEDULE](#-refheading-toc43945-784232322) based on this philosophy.
The first segment of the example shows the start of the [SCHEDULE](#-refheading-toc43945-784232322) section and the group definitions and controls. Here there are group controls only at the FIELD level and there are both production and injection rate targets, as well as water and liquid handling constraints applied on the FIELD level. These keywords are activated from the start of the simulation as set by the [START](#-refheading-toc39156-327352552) keyword in the [RUNSPEC](#-refheading-toc55591-1778172979) section, in this case January 1, 2020.
...
The next segment covers the well specification and includes the loading of the VFP tables via include files, declaring the wells using the [WELSPECS](#-refheading-toc268463-1366622701) keyword, and connecting the wells to the reservoir using the [COMPDAT](#-refheading-toc97651-3261743917) keyword. In addition, the [COMPLUMP](#-refheading-toc97655-3261743917) keyword is invoked to assign the well connections to  well completions. 
...
Finally, the [WCONPROD](#-refheading-toc146754-4203985108) keyword is used to define the operating conditions for the gas producers \(GP01 and GP02\) and to assign the VFP tables to the producing wells. Whereas the [WCONINJE](#-refheading-toc146750-4203985108) keyword performs a similar function for the single gas injector, GI01.
...
The final part of the well specification segment is shown below and sets the well efficiency for the wells via the [WEFAC](#-refheading-toc48856-327352552) keyword, the producing wells economic limits via the [WECON](#-refheading-toc134884-2055188184) keyword,  and then advances the simulation to January 25, 2020. Thus, there is no production up to January 25, 2020 as all the wells are shut\-in.                                                                                                                                                                                                                                
```eclipse
--
...
On January 25, 2020 yearly restarts are requested \([RPTRST](#-refheading-toc210154-2884651453)\) together with various printed reports \([RPTSCHED](#-refheading-toc268459-1366622701)\), as well as the maximum time step size of 31 days being set via the [TUNING](#-refheading-toc146744-4203985108) keyword. More importantly, well GP01 is opened together with completion number one to put the well on production. Note that one needs to open both the well and the completion; hence, the two lines in the [WELOPEN](#-refheading-toc268461-1366622701) keyword. 
```eclipse
-- ------------------------------------------------------------------------------
...
On February 1, 2020 further [SCHEDULE](#-refheading-toc43945-784232322) keywords are processed.  The first keyword is the [RPTSCHED](#-refheading-toc268459-1366622701) keyword that in this case switches off all reports printed to the \*.PRT file, as only annual reports are required for this particular model.
Next the gas producer GP02 is put on production using two statements in the [WELOPEN](#-refheading-toc268461-1366622701) keyword, and the simulator is requested to advance to report time step of March 1, 2020.
```eclipse
--
--       DEFINE SCHEDULE SECTION REPORT OPTION
--
...
### <a id="-refheading-toc311722-2754225602"></a>History Matching Runs
...
The [WCONINJH](#-refheading-toc146752-4203985108) keyword has similar functionality as the [WCONHIST](#-refheading-toc134880-2055188184) keyword, but is used for history matching injection wells instead.
More recent developments in history matching incorporate Assisted History Matching \(“AHM”\) techniques 1
       History Matching and Uncertainty Quantification: Multi\-objective Particle Swarm Optimisation Approach \(SPE 143067\), L. Mohamed, M. Christie, V. Demyanov, Vienna, Austria, 23–26 May 2011.<sup>, </sup>2
       Field\-Scale Assisted History Matching Using a Systematic, Massively Parallel Ensemble Kalman Smoother Procedure \(SPE00182617\), Binghuai Lin, Paul I Crumpton, and Ali H. Dogru, Society of Petroleum Engineers \(February 2017\).<sup>,</sup> 3
       Enhancing the Geological Models Consistency in Ensemble\-Based History Matching \-  An Integrated Approach \(SPE00186049\),  A. Perrone, F Pennadoro,  A Tiani, E Della Rossa, and J. Saetrom, Society of Petroleum Engineers \(May,  2017\).<sup>,</sup> 4
       Correlation\-Based Adaptive Localization for Ensemble\-Based History Matching \- Applied to the Norne Field Case Study \(SPE00191305\), Xiaodong Luo, Rolf Lorentzen, Randi Valestrand, and Geir Evensen, , Society of Petroleum Engineers \(April,  2018\).<sup>, </sup><sup>5
        Rapid Forecast Calibration Using Nonlinear Simulation Regression with Localization \(SPE\-193845\-MS\), Jincong He, Wenyue Sun, and Xian\-Huan Wen, SPE Reservoir Simulation Conference held in Galveston, Texas, USA, 10–11 April 2019.</sup><sup>, </sup><sup>6
        Multilevel Strategies and Geological Parameterizations for History Matching Complex Reservoir Models \(SPE\-193895\-MS\), Yimin Liu and Louis J. Durlofsky,SPE Reservoir Simulation Conference held in Galveston, Texas, USA, 10–11 April 2019.</sup>and 7
       Improved Estimation and Forecast Through Model Error Estimation – Norne Field Example \(IPTC\-19142\-MS\), Minjie Lu and Yan Chen,  International Petroleum Technology Conference held in Beijing, China, 26 – 28 March 2019. in which various statistical techniques are applied in specialized software to match the field data by the user supplying the parameters to vary, the range of the parameters values, and the objective function to minimize.  This type of software can also generate a range of “reasonable” history match models that can then be used to quantify the uncertainty range in the prediction phase.

### <a id="-refheading-toc311724-2754225602"></a>Prediction Runs
...
#### <a id="-refheading-toc716517-3964674244"></a>Example
...
From December 1, 2019 to January 1, 2020 the three oil producing wells will be under history matching control with the oil rate being the target rate for the wells. On January 1, 2020 the wells will be converted to normal producers as the [WELTARG](#-refheading-toc134888-2055188184) keyword changes a well status from a history match well to a regular well.  At the same time the [WELTARG](#-refheading-toc134888-2055188184) keyword sets all the wells to have their current FBHP as a constraint, by setting the target BHP values to the default values, which effectively makes the FBHP a target as it is now the “active” constraint.

The alternative keyword for implementing the same approach is the [WELCNTL](#-refheading-toc134886-2055188184) keyword, but unfortunately this is currently not implemented in OPM Flow.

### <a id="-refheading-toc423069-1189575515"></a>Well Productivity

Throughout this section the equations are presented in terms of oil production or liquid flow \(i.e., oil and water\). Note that similar equations exists for gas flow in terms of p<sup>2</sup> and pseudo pressure formulations \(ψ\(p\)\).
...
| $$ q\, = \, ( 0.001127 ) \, ( 2π ) \,\frac{\mathit{kh}}{μ} [ \frac{{p}_{e}\, - \,{p}_{w}}{ ( \mathrm{ln}\, ( \frac{{r}_{e}}{{r}_{w}} ) \, + \,S ) } ] $$ | \(12.1\) |
| --- | --- |

where 	
q	= production rate \(rb/day\),
p<sub>e</sub>	= pressure at the drainage radius of the well \(psia\),
p<sub>w</sub>	= bottom\-hole well pressure \(psia\),
r<sub>e</sub>	= drainage radius \(feet\),
r<sub>w</sub>	= wellbore radius \(feet\),
...
| $$ {q}_{o}\, = \, ( 0.001127 ) \, ( 2π ) \,\frac{{\mathit{kk}}_{\mathit{ro}}\,h}{{B}_{o}\,{μ}_{o}} [ \frac{{p}_{e}\, - \,{p}_{w}}{ ( \mathrm{ln}\, ( \frac{{r}_{e}}{{r}_{w}} ) \, + \,S ) } ] $$ | \(12.2\) |
| --- | --- |

If the modified radial flow equation is written for each perforated interval and summed over all intervals, the resulting equation is the one used to calculate oil production rates in the simulator. One way to specify the flow capacities of wells is to enter r<sub>e</sub>, r<sub>w</sub>, S, and p<sub>w</sub> values for the well, with the kh values for each perforated interval. The simulator then determines the other parameter values from the calculated pressure and saturation distributions.
Because the pressure at the drainage radius is not readily determined in the simulator, it substitutes p<sub>b</sub> \(a representative pressure of each perforation grid block\) for p. That is readily accommodated by replacing the drainage radius by a probe radius as defined by Peaceman8
        Peaceman, D.W. "Interpretation of Well\-Block Pressures in Numerical Reservoir Simulation with Nonsquare Grid Blocks and Anisotropic Permeability."  SPE 10528 presented at the Sixth SPE Symposium on Reservoir Simulation, New Orleans, \(February 1982\), pp. 553\-569. for a vertical well:
| $$ {r}_{b}\, = \,0.28\,\frac{{ [ \sqrt{\frac{{K}_{y}}{{K}_{x}}}\,Δ{x}^{2}\, + \,\sqrt{\frac{{K}_{x}}{{K}_{y}}}\,Δ{y}^{2} ] \,}^{0.5}}{{ ( \frac{{K}_{y}}{{K}_{x}} ) }^{0.25}\, + \,{ ( \frac{{K}_{x}}{{K}_{y}} ) }^{0.25}} $$ | \(12.3\) |
| --- | --- |

where	
K<sub>x</sub> and K<sub>y</sub>		= 	x\- and y\- direction absolute permeabilities, and
Δx and Δy		= 	grid block sizes \(feet\).

The simulator then uses the modified radial flow equation, written for each connection:
| $$ {q}_{o}\, = \,\frac{ ( 0.001127 ) \, ( 2π ) }{ ( \mathrm{ln}\, ( \frac{{r}_{b}}{{r}_{w}} ) \, + \,S ) } ( \frac{k{k}_{\mathit{ro}}h}{{B}_{o}\,{μ}_{o}}\, ( \,{p}_{b}\, - \,{p}_{w}\, )  ) $$ | \(12.4\) |
| --- | --- |

The connection term on the [COMPDAT](#-refheading-toc97651-3261743917) keyword in the [SCHEDULE](#-refheading-toc43945-784232322) section, CONFACT, is simply the transmissibility portion of equation \(12.4\) for a given connection that is:
| $$ {T}_{\mathit{xy}}\, = \,\frac{ ( 0.001127 ) \, ( 2π )  ( \mathit{kh} ) }{ ( \mathrm{ln}\, ( \frac{{r}_{b}}{{r}_{w}} ) \, + \,S ) } $$ | \(12.5\) |
| --- | --- |

And the oil phase mobility term is defined as:
| $$ {M}_{o}\, = \, ( \frac{{k}_{\mathit{ro}}}{{B}_{o}\,{μ}_{o}} ) $$ | \(12.6\) |
| --- | --- |
...
| $$ {q}_{o}\, = \, \sum 1N ( {T}_{\mathit{xy}}\,{M}_{o}\, ( {p}_{b}\, - \,{p}_{w} )  ) $$ | \(12.7\) |
| --- | --- |
...
| $$ {q}_{o}\, = \,PI ( \,{p}_{e}\, - \,{p}_{w}\, ) $$ | \(12.8\) |
| --- | --- |

or in terms of the productivity index:
| $$ PI\, = \,\frac{{q}_{o}}{ ( \,{p}_{e}\, - \,{p}_{w}\, ) } $$ | \(12.9\) |
| --- | --- |

It is significant that this equation for the Productivity Index is written in terms of the pressure at the well drainage radius, p<sub>e</sub>, rather than grid block pressure, p<sub>b</sub>.  It must be written this way since PI is a property that is determined by field production tests. One can also relate the above equation to the multilayered extension of the steady\-state radial flow equation \(12.7\), to give the grid block Productivity Index:
| $$ PI\, = \,\frac{{q}_{o}}{ ( {p}_{b}\, - \,{p}_{w} ) }\, = \, \sum 1N ( {T}_{\mathit{xy}}\,{M}_{o}\, ) $$ | \(12.10\) |
| --- | --- |
...
| $$ PI\, = \,{ \sum n = 1N ( {T}_{\mathit{xy}}\,{M}_{o}\, ( \frac{\mathrm{ln}\, ( \frac{{r}_{b}}{{r}_{w}} ) \, + \,S}{\mathrm{ln}\, ( \frac{{r}_{e}}{{r}_{w}} ) \, + \,S} )  ) }_{n} $$ | \(12.11\) |
| --- | --- |

Since the drainage radius, r<sub>e</sub>, is generally different from the grid block radius, r<sub>b</sub>, the ratio of logarithms may be significant; hence, the use of the PI option normally requires specification of both r<sub>e</sub> and r<sub>b</sub>.
Equation \(12.11\) is used to calculate the PI for the well and is used to print the PI on the WELLS production report requested via the [RPTSCHED](#-refheading-toc268459-1366622701) keyword in the [SCHEDULE](#-refheading-toc43945-784232322) section. Note that for gas wells with non\-zero D\-factors, the non\-Darcy skin factor is added to to the skin \(S\) term in equation \(12.11\) in both the numerator and the denominator.  Secondly, r<sub>e </sub>is often undetermined or unknown and it is common to default the DRADIUS parameter on the [WELSPECS](#-refheading-toc268463-1366622701) keyword, in the [SCHEDULE](#-refheading-toc43945-784232322) section. If the well drainage radius \(DRADIUS\) is defaulted then equation \(12.11\) simplifies to:
| $$ PI\, = \,{ \sum n = 1N ( {T}_{\mathit{xy}}\,{M}_{o}\, ) }_{n} $$ | \(12.12\) |
| --- | --- |
