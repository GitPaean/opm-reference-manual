...
| 31 | \--enable-dry-run | A boolean value that specifies if the simulation should actually run (true), or just check the input deck (false).<br>This option is equivalent to activating the [NOSIM](#__RefHeading___Toc251745_4250154414) keyword in the [RUNSPEC](#__RefHeading___Toc55591_1778172979) section of the input deck (see section [[Error: Reference source not found](#anchor-1)[Error: Reference source not found](#anchor-1)](#5.2.32.NOSIM – Activates the No Simulation Mode for Data File Checking|outline)).<br>The default value of "auto' will use whatever is stipulated in the input deck via the [NOSIM](#__RefHeading___Toc251745_4250154414) keyword. | "auto" |
...
| 42 | \--enable-tuning | A boolean value that instructs OPM Flow to read the time stepping and convergence parameters from the [TUNING](#__RefHeading___Toc146744_4203985108) keyword in the input deck, if set to true.<br>Note that only some of the [TUNING](#__RefHeading___Toc146744_4203985108) keyword parameters are processed. | false |
...
| 66 | \--linear-solver | A defined quoted character string that sets the configuration of the linear solver, valid values are:<br>1)  cprw[^1] [^2] [^3] (default),<br>2)  ilu0,<br>3)  dilu,<br>4)  cpr (an alias for cprw),<br>5)  cpr_quasiimpes,<br>6)  cpr_trueimpes,<br>7)  cpr_trueimpesanalytic,<br>8)  amg[^4],<br>9)  hybrid (experimental), or<br>10) a file name that has the extension ".json", that contains the linear solver configuration parameters.<br>Option (1) extends the existing Constrained Pressure Residual ("CPR") preconditioner to include wells. This option can also be invoked via the [CPR](#__RefHeading___Toc27871_3671211675) keyword in the [RUNSPEC](#__RefHeading___Toc55591_1778172979) section; however, the command line parameter takes precedence.<br>For option (10) one enters a character string enclosed in quotes that defines the filename of a JSON configuration file for the flexible linear solver system. In this case the file extension should be '.json'.<br>Note that the \*.PRT file contains the "Property tree for the linear solver" listing, which is the JSON specification of the current case, and can be used to configure a user specific linear solver JSON file. | \"cprw\" |
...
| 140 | \--parsing-strictness | Set strictness of parsing process. Available options are:<br>1)  \"normal\": stop for critical errors, and for unsupported keywords that would would change the simulator results if supported.<br>2)  \"high\": stop for all errors, that is even for unsupported keywords that do not effect the results, for example [ECHO](#__RefHeading___Toc605466_3199477706) and [NOECHO](#__RefHeading___Toc605482_3199477706).<br>3)  \"low\": same as normal, except do not stop due to unsupported keywords that would change the simulator results if supported, and even if marked critical.<br>Default: \"normal\" | \"normal\" |
...
| 160 | \--sched-restart | A boolean value that determines for a restart run if the case should initialize wells and groups from the historical [SCHEDULE](#__RefHeading___Toc43945_784232322) section (true), or from the well and group data on the restart file (false).<br>Note that the commercial simulator always uses data from the restart file (false). | false |
...
*<a id="REF_TABLE_OPM_FLOW_2025_04___APPENDIX_E"></a>Table A.1: OPM Flow 2025-04 Command Line Options*
...
| 14 | \--cpr-reuse-setup | A positive integer that defines if the [CPR](#__RefHeading___Toc27871_3671211675) solver should re-use the AMG setup. Valid options are:<br>0.  Recreate the preconditioner for every linear solve.<br>1.  Recreate once every time step.<br>2.  Recreate if last linear solve took more than 10 iterations.<br>3.  Never recreate.<br>4.  Recreate every N linear solves, where N is the parameter *\--cpr-reuse-interval*.<br>Changed the default value from three to four. | 4 |
...
| 39 | \--enable-tuning | A boolean value that instructs OPM Flow to read the time stepping parameters from the [[Error: Reference source not found](#anchor-2)](#11.3.48.TUNING - Numerical Tuning Control|outline) keyword in the input deck, if set to true.<br>Note that only the first record of the [TUNING](#__RefHeading___Toc146744_4203985108) keyword is processed. | false |
...
| 62 | \--linear-solver | A defined quoted character string that sets the configuration of the linear solver, valid values are:<br>1)  cprw[^11] [^12] [^13] (default),<br>2)  ilu0,<br>3)  dilu,<br>4)  cpr (an alias for cprw),<br>5)  cpr_quasiimpes,<br>6)  cpr_trueimpes,<br>7)  cpr_trueimpesanalytic,<br>8)  amg[^14],<br>9)  hybrid (experimental), or<br>10) a file name that has the extension ".json", that contains the linear solver configuration parameters.<br>Option (1) extends the existing Constrained Pressure Residual ("CPR") preconditioner to include wells. This option can also be invoked via the [CPR](#__RefHeading___Toc27871_3671211675) keyword in the [RUNSPEC](#__RefHeading___Toc55591_1778172979) section; however, the command line parameter takes precedence.<br>For option (10) one enters a character string enclosed in quotes that defines the filename of a JSON configuration file for the flexible linear solver system. In this case the file extension should be '.json'.<br>Note that the \*.PRT file contains the "Property tree for the linear solver" listing, which is the JSON specification of the current case, and can be used to configure a user specific linear solver JSON file. | \"cprw\" |
...
| 166 | \--time-step-control-file-name | A character string enclosed in quotes that specifies a filename where time steps are specified. The default is the character string timesteps.<br>For instance time steps can be generated by the ecl_summary application in libecl as per the following UNIX command line:<br>path_to_libecl_applications/ecl_summary DECK TIME \> filename<br>Where:<br>DECK is the name of the data deck you want to get the time steps from,<br>[TIME](#__RefHeading___Toc1252966_4250154414) tells the application to return the timing for the run, and<br>\"filename\" is the name of the file the times are piped to. | "timesteps" |
...
*<a id="REF_TABLE_OPM_FLOW_2023_04___OPTIONS_2_2"></a>Table A.1: OPM Flow 2024-10 Command Line Options*
...
| 9 | \--cpr-reuse-setup | A positive integer that defines if the [CPR](#__RefHeading___Toc27871_3671211675) solver should re-use the AMG setup. Valid options are:<br>0.  Recreate the preconditioner for every linear solve.<br>1.  Recreate once every time step.<br>2.  Recreate if last linear solve took more than 10 iterations.<br>3.  Never recreate.<br>4.  Recreate every N linear solves, where N is the parameter *\--cpr-reuse-interval*.<br>Changed the default value from three to four. | 4 |
...
| 40 | \--enable-tuning | A boolean value that instructs OPM Flow to read the time stepping parameters from the [[Error: Reference source not found](#anchor-2)](#11.3.48.TUNING - Numerical Tuning Control|outline) keyword in the input deck, if set to true.<br>Note that only the first record of the [TUNING](#__RefHeading___Toc146744_4203985108) keyword is processed. | false |
...
*<a id="REF_TABLE_OPM_FLOW_2023_04___OPTIONS_2_2"></a>Table A.3: OPM Flow 2024-04 Command Line Options*
...
| 55 | \--linear-solver | A defined quoted character string that sets the configuration of the linear solver, valid values are:<br>1)  ilu0 (default),<br>2)  cprw or cpr (an alias for cprw),[^31] [^32] and [^33]<br>3)  cpr_quasiimpes,<br>4)  cpr_trueimpes,<br>5)  amg,[^34]<br>6)  hybrid (experimental), or<br>7)  a file name that has the extension ".json", that contains the linear solver configuration parameters.<br>Option (2) extends the existing Constrained Pressure Residual ("CPR") preconditioner to include wells. This option can also be invoked via the [CPR](#__RefHeading___Toc27871_3671211675) keyword in the [RUNSPEC](#__RefHeading___Toc55591_1778172979) section; however, the command line parameter takes precedence.<br>For option (7) one enters a character string enclosed in quotes that defines the filename of a JSON configuration file for the flexible linear solver system. In this case the file extension should be '.json'.<br>Note that the \*.PRT file contains the "Property tree for the linear solver" listing, which is the JSON specification of the current case, and can be used to configure a user specific linear solver JSON file.<br>The option \"cpr\" now is an alias for \"cprw\" instead of \"cpr_trueimpes\". | "ilu0" |
